...
# parameters
firsTime = False
# flag that if true, will load the raw images from the directories and
# extract the features and prepare the inputs for training.
startFromClosest = False
# if ture, for every target, we would select the base to be the closest instance
# from the other class in the test set. Closeness is measured by L2 distance
# in feature space reps
threshold = 3.5
# threshold for L2 distance in feature space


if not startFromClosest :
    # these are some preferred images as base instances which will be used for
    # generating poison instances for the attacks
...
if firsTime:
    ...
    X_tr, X_test, X_inp_tr, X_inp_test, Y_tr, Y_test = load_bottleNeckTensor_data(directory='./Data/final_images/',
                                                                                  saveEm=True)
...
for i in range(len(X_test)):
    ...
    while (diff > threshold) and (counter < maxTriesForOptimizing):
        if Y_test[i] == 1 and counter < len(
                class_dog_base_id_in_test):  # if target is fish, the poison base should be a dog
